...
for sub = 1:subject
    ...
    for ses = 1:session
        ...
        % Read in z-statistic from subject level results
        ...
        for run = 1:run_total
            ...
            % Read in z-statistic from run level results
...
%% Calculate mean and std of z-statistic per run
...
for a = 1:2
    for b = 2:4
        ...
        if h
            fprintf('Paired-sample t-test of z-statistic in session %d, run%d and run%d has p= %f and Significant! \n', a, 1, b, p)
            % plot([(a-1)*4+1 (a-1)*4+b], [34+b 34+b], 'black'); hold on;
            % text(((a-1)*4+1+(a-1)*4+b)/2, 34.25+b, '*')
        else
            fprintf('Paired-sample t-test of z-statistic in session %d, run%d and run%d has p= %f \n', a, 1, b, p)
        end
    end
end
...
for b = 1:4
    ...
    if h
        fprintf('Paired-sample t-test of z-statistic  between session 1 and 2 for run%d has p= %f and Significant! \n',b, p)
        % plot([(a-1)*4+1 (a-1)*4+b], [34+b 34+b], 'black'); hold on;
        % text(((a-1)*4+1+(a-1)*4+b)/2, 34.25+b, '*')
    else
        fprintf('Paired-sample t-test of z-statistic between session 1 and 2 for run%d has p= %f \n', b, p)
    end
end
