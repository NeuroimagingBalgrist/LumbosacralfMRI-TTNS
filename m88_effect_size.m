...
for sub = 1:subject
    ...
    for ses = 1:session
        ...
        % Read in selected mask and limit to neurological levels L2-L5
        ...
        for run = 1:run_total
            ...
            beta2_image = niftiread(sprintf('%s/stats/pe2.nii.gz', pathRun));
            ...
            expvar_image_1 = zeros(size(res4d_image));    % image series containing the paradigm convoluted with the HRF
            expvar_array_1 = squeeze(design_matrix(:,1));
            for vol = 1:size(res4d_image, 4)
                expvar_image_1(:,:,:,vol) = expvar_array_1(vol);
            end

            expvar_image_2 = zeros(size(res4d_image));    % image series containing the paradigm convoluted with the HRF
            expvar_array_2 = squeeze(design_matrix(:,2));
            for vol = 1:size(res4d_image, 4)
                expvar_image_2(:,:,:,vol) = expvar_array_2(vol);
            end

            signal_image = zeros(size(res4d_image));    % reconstructed signal from GLM
            for vol = 1:size(signal_image,4)
                signal_image(:,:,:,vol) = (beta0_image(:,:,:) + beta1_image(:,:,:).*expvar_image_1(:,:,:,vol) + beta2_image(:,:,:).*expvar_image_2(:,:,:,vol) + res4d_image(:,:,:, vol));
            end
            ...
            while a <= length(isodd)
                if isodd(a) > isodd(a-1)
                    a = a + 6;    % discard first 6 volumes after task onset
                elseif isodd(a) == 1
                    istask(a) = 1;
                    a = a + 1;
                else
            ...
            while a <= length(iseven)
                if iseven(a) > iseven(a-1)
                    a = a + 6;    % discard first 6 volumes after rest onset
                elseif iseven(a) == 1
                    isrest(a) = 1;
                    a = a + 1;
                else
...
plot(expvar_array_1)
axis([0 length(expvar_array_1) -9 9])
hold off
...
ax = gca;
yticks(-0.6:0.2:1.0)
yticklabels(compose('%.1f', -0.6:0.2:1.0))
ax.LineWidth = 1.5;
ax.FontSize = 30;
