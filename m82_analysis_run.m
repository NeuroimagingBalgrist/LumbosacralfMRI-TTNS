...
result_p_image = struct( ...
    'p_4run_10min', {double(niftiread(sprintf('%s/21_grplvl_randomise_varsm-2/%dof25sub/%s/%s_%dof25sub-mean_%s_tfce_corrp_tstat1_cr.nii',pathRes, sub, "4run_10min", ses, sub, "4run_10min")))}, ...
    'p_4run_8min', {double(niftiread(sprintf('%s/21_grplvl_randomise_varsm-2/%dof25sub/%s/%s_%dof25sub-mean_%s_tfce_corrp_tstat1_cr.nii',pathRes, sub, "4run_8min", ses, sub, "4run_8min")))}, ...
    'p_3run_10min', {double(niftiread(sprintf('%s/21_grplvl_randomise_varsm-2/%dof25sub/%s/%s_%dof25sub-mean_%s_tfce_corrp_tstat1_cr.nii',pathRes, sub, "3run_10min", ses, sub, "3run_10min")))}, ...
    'p_4run_6min', {double(niftiread(sprintf('%s/21_grplvl_randomise_varsm-2/%dof25sub/%s/%s_%dof25sub-mean_%s_tfce_corrp_tstat1_cr.nii',pathRes, sub, "4run_6min", ses, sub, "4run_6min")))}, ...
    'p_3run_8min', {double(niftiread(sprintf('%s/21_grplvl_randomise_varsm-2/%dof25sub/%s/%s_%dof25sub-mean_%s_tfce_corrp_tstat1_cr.nii',pathRes, sub, "3run_8min", ses, sub, "3run_8min")))}, ...
    'p_2run_10min', {double(niftiread(sprintf('%s/21_grplvl_randomise_varsm-2/%dof25sub/%s/%s_%dof25sub-mean_%s_tfce_corrp_tstat1_cr.nii',pathRes, sub, "2run_10min", ses, sub, "2run_10min")))}, ...
    'p_3run_6min', {double(niftiread(sprintf('%s/21_grplvl_randomise_varsm-2/%dof25sub/%s/%s_%dof25sub-mean_%s_tfce_corrp_tstat1_cr.nii',pathRes, sub, "3run_6min", ses, sub, "3run_6min")))}, ...
    'p_2run_8min', {double(niftiread(sprintf('%s/21_grplvl_randomise_varsm-2/%dof25sub/%s/%s_%dof25sub-mean_%s_tfce_corrp_tstat1_cr.nii',pathRes, sub, "2run_8min", ses, sub, "2run_8min")))}, ...
    'p_2run_6min', {double(niftiread(sprintf('%s/21_grplvl_randomise_varsm-2/%dof25sub/%s/%s_%dof25sub-mean_%s_tfce_corrp_tstat1_cr.nii',pathRes, sub, "2run_6min", ses, sub, "2run_6min")))}, ...
    'p_2nd_half', {double(niftiread(sprintf('%s/21_grplvl_randomise_varsm-2/%dof25sub/%s/%s_%dof25sub-mean_%s_tfce_corrp_tstat1_cr.nii',pathRes, sub, "2run_10min_2ndhalf", ses, sub, "2run_10min_2ndhalf")))});
...
result_t_image = struct( ...
    't_4run_10min', {double(niftiread(sprintf('%s/21_grplvl_randomise_varsm-2/%dof25sub/%s/%s_%dof25sub-mean_%s_tstat1_cr.nii',pathRes, sub, "4run_10min", ses, sub, "4run_10min")))}, ...
    't_4run_8min', {double(niftiread(sprintf('%s/21_grplvl_randomise_varsm-2/%dof25sub/%s/%s_%dof25sub-mean_%s_tstat1_cr.nii',pathRes, sub, "4run_8min", ses, sub, "4run_8min")))}, ...
    't_3run_10min', {double(niftiread(sprintf('%s/21_grplvl_randomise_varsm-2/%dof25sub/%s/%s_%dof25sub-mean_%s_tstat1_cr.nii',pathRes, sub, "3run_10min", ses, sub, "3run_10min")))}, ...
    't_4run_6min', {double(niftiread(sprintf('%s/21_grplvl_randomise_varsm-2/%dof25sub/%s/%s_%dof25sub-mean_%s_tstat1_cr.nii',pathRes, sub, "4run_6min", ses, sub, "4run_6min")))}, ...
    't_3run_8min', {double(niftiread(sprintf('%s/21_grplvl_randomise_varsm-2/%dof25sub/%s/%s_%dof25sub-mean_%s_tstat1_cr.nii',pathRes, sub, "3run_8min", ses, sub, "3run_8min")))}, ...
    't_2run_10min', {double(niftiread(sprintf('%s/21_grplvl_randomise_varsm-2/%dof25sub/%s/%s_%dof25sub-mean_%s_tstat1_cr.nii',pathRes, sub, "2run_10min", ses, sub, "2run_10min")))}, ...
    't_3run_6min', {double(niftiread(sprintf('%s/21_grplvl_randomise_varsm-2/%dof25sub/%s/%s_%dof25sub-mean_%s_tstat1_cr.nii',pathRes, sub, "3run_6min", ses, sub, "3run_6min")))}, ...
    't_2run_8min', {double(niftiread(sprintf('%s/21_grplvl_randomise_varsm-2/%dof25sub/%s/%s_%dof25sub-mean_%s_tstat1_cr.nii',pathRes, sub, "2run_8min", ses, sub, "2run_8min")))}, ...
    't_2run_6min', {double(niftiread(sprintf('%s/21_grplvl_randomise_varsm-2/%dof25sub/%s/%s_%dof25sub-mean_%s_tstat1_cr.nii',pathRes, sub, "2run_6min", ses, sub, "2run_6min")))}, ...
    't_2nd_half', {double(niftiread(sprintf('%s/21_grplvl_randomise_varsm-2/%dof25sub/%s/%s_%dof25sub-mean_%s_tstat1_cr.nii',pathRes, sub, "2run_10min_2ndhalf", ses, sub, "2run_10min_2ndhalf")))});
...
line_color = ["#A2142F" "#EDB120" "#0072BD"];
colororder(line_color)

if strcmp(ses,'ses-02')

    hline = findobj(gcf,'Type','line');
    set(hline,'LineStyle','-','Marker','o','MarkerSize',8)
    ...
    for k = 1:numel(hline)
        ...
        set(hline(k),'MarkerIndices',sort(idx));
...
if strcmp(ses,'ses-02')
    ...
    for k = 1:numel(hline)
        ...
        % Use all points that are not at x=0
        idy = find(abs(y) > 1e-10);

        % Add only occasional markers on the y=0 segment
        idy0 = find(abs(y) <= 1e-10);

        if ~isempty(idx0)
            idy = [idy(:); idy0(1:10:end)'];
        end

        set(hline(k),'MarkerIndices',sort(idy));
